<?php
    ...
    if (!isset($_GET['mes'])){
        ...
        $resultUsuarios=mysqli_query($conexion,"call ContHoras(0,$areaID);");
        $mesNombre=$mes;
    }
    else {
        ...
        $resultUsuarios=mysqli_query($conexion,"call ContHoras($mes,$areaID);");
        switch($mes){
            case "1":
                    $mesNombre="Enero";
                    break;
...
<?php require 'template/head.php'; ?>
        ...
        <title>Grafico Cantidad de Horas</title>
    </head>
    <body>
        ...
        <?php include 'template/icon-logo.php';?>
            <?php include 'template/titulos-horas.php';?>
                <div>
                    ...
                    <select class="form-control" id="career" onchange="changeMonthHoras(this)">
                ...
                <?php
                    include("cn.php");
                    if (!isset($_GET['mes'])){
                        $pri=mysqli_query($conexion,"call ContHoras(0,$areaID);");
                    }
                    else {
                        $pri=mysqli_query($conexion,"call ContHoras($mes,$areaID);");
                    }
