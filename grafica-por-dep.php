<?php
    ...
    if (!isset($_GET['mes'])){
        ...
        $resultUsuarios=mysqli_query($conexion,"call ContAreaPor($numMes);");
        $mesNombre=$mes;
    }
    else {
        ...
        $resultUsuarios=mysqli_query($conexion,"call ContAreaPor($mes);");
        switch($mes){
            case "1":
                    $mesNombre="Enero";
                    break;
...
<?php require 'template/head.php'; ?>
        ...
        <title>Grafico Pocentaje Departamento</title>
    </head>
    <body>
    <?php include 'template/nav.php'; ?>
        ...
        <?php include 'template/icon-logo.php';?>
            ...
            <div>
                ...
                <select class="form-control" id="career" onchange="changeMonthAreasPor(this)">
        ...
        <div class="container">
            <table id="tabla-horas" class="table col-xl-2 mr-5 text-center mt-3">
                ...
                <?php
                    include("cn.php");
                    if (!isset($_GET['mes'])){
                        $pri=mysqli_query($conexion,"call ContAreaPor($numMes);");
                    }
                    else {
                        $pri=mysqli_query($conexion,"call ContAreaPor($mes);");
                    }
